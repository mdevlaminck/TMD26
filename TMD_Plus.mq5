...
#include <TMD+/Symbols.mqh>
#include <TMD+/Utils.mqh>
#include <TMD+/Trend.mqh>
#include <TMD+/Strength.mqh>
#include <TMD+/Exposure.mqh>
#include <TMD+/Entry.mqh>
#include <TMD+/Risk.mqh>
#include <TMD+/GridManager.mqh>
#include <TMD+/Exit.mqh>
...
void LogTradeEvent(const string msg)
{
   Print("TMD_Lite: ", msg);
}
...
void ProcessEntryLayer()
{
   for(int i = 0; i < ArraySize(g_pairs); i++)
   {
      ...
      if(TryOpenInitialBasket(g_trade,
                              ...
                              InpFilterByMagicOnly))
      {
         LogTradeEvent("OPEN | " + EntryCandidateToText(candidate));
      ...
      if(hasBuy)
      {
                            ...
                            reason))
         {
            ...
            if(BuildBasketInfo(symbol, ORDER_TYPE_BUY, buyBasketAfter, InpMagicBase, InpFilterByMagicOnly))
               LogTradeEvent("ADD | BUY | " + BasketExpandToText(buyBasket, buyBasketAfter,
                                                  InpSetupTF,
                                                  InpATRPeriod,
                                                  InpGridStepATR,
                                                  InpMinSecondsBetweenAdds,
               ...
               LogTradeEvent("ADD | BUY | " + BasketExpandToText(buyBasket, buyBasket,
                                                  InpSetupTF,
                                                  InpATRPeriod,
                                                  InpGridStepATR,
                                                  InpMinSecondsBetweenAdds,
      ...
      if(hasSell)
      {
                            ...
                            reason))
         {
            ...
            if(BuildBasketInfo(symbol, ORDER_TYPE_SELL, sellBasketAfter, InpMagicBase, InpFilterByMagicOnly))
               LogTradeEvent("ADD | SELL | " + BasketExpandToText(sellBasket, sellBasketAfter,
                                                  InpSetupTF,
                                                  InpATRPeriod,
                                                  InpGridStepATR,
                                                  InpMinSecondsBetweenAdds,
               ...
               LogTradeEvent("ADD | SELL | " + BasketExpandToText(sellBasket, sellBasket,
                                                  InpSetupTF,
                                                  InpATRPeriod,
                                                  InpGridStepATR,
                                                  InpMinSecondsBetweenAdds,
...
void RunEngineCycle()
{
   ...
   // Final refresh
   UpdateAllPairStates(false);
   FinalizePendingBasketCloses(g_closeAudit, g_debug, InpMagicBase, InpFilterByMagicOnly);
   RenderDashboard();
...
int OnInit()
{
   ...
   InitializePairs();
   InitializeBasketCloseAudit(g_closeAudit, g_brokerSymbols);
